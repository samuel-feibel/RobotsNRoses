...
void loop() {
  ...
  if(true){//l_light < GREY && r_light < GREY) { // at intersection update gui
    right_wall = (dir + 1) % 4;
    left_wall  = (dir + 3) % 4;
...
void check_for_hat() {
  ...
  cli();  // UDRE interrupt slows this way down on arduino1.0
    ...
    if(fft_log_out[40] > 100 || fft_log_out[41] > 100 || fft_log_out[42] > 100 || fft_log_out[43] > 100 || fft_log_out[44] > 100 || fft_log_out[45] > 100 || fft_log_out[46] > 100){
      //hat_detected = true; Serial.println("HAT DETECTED");
    } else {
      //hat_detected = false; Serial.println("NO HAT DETECTED");
    }
    TIMSK0 = timsko;
    ADMUX = admux;
    DIDR0 = didro;
